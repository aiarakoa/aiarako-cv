...
export function CVProvider({ children }) {
  ...
  const [sectionsPerGroupByPresentationMode, setSectionsPerGroupByPresentationMode] =
    useState(null);
  ...
  const [languages, setLanguages] = useState(
    /** @type {Array<{code:string,label:string,dir?:'ltr'|'rtl'}>|null} */ (null),
  );
  ...
  async function loadManifestOnce() {
    ...
    const firstLoadPresentationMode = window.matchMedia('(min-width: 1200px)').matches
      ? 'laptop'
      : 'mobile';
    setPresentationMode(firstLoadPresentationMode);
    setSectionsPerGroup(
      manifest.sectionsPerGroup.find(
        (secPerGroup) => secPerGroup.presentationMode === firstLoadPresentationMode,
      ).sectionsPerGroup,
    );
  }
  ...
  async function loadCVForLanguage(langCode) {
    ...
    json.articles.forEach((article) => {
      ...
      article.sections.forEach((section) =>
        sectionsMap.set(section.key, (({ key, ...sect }) => sect)(section)),
      );
    });
  ...
  return (
    <CVContext.Provider
      value={{
        ...
        sectionsPerGroup,
        setSectionsPerGroup,
        sectionGroupIndexByArticle,
        setSectionGroupIndexByArticle,
        sectionsPerGroupByPresentationMode,
        setSectionsPerGroupByPresentationMode,
        activeLink,
        setActiveLink,
        presentationModeList,
        setPresentationModeList,
        presentationMode,
        setPresentationMode,
        navComponents,
        resizeTick,
