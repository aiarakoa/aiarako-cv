...
function GenArticle(props) {
  ...
  useLayoutEffect(() => {
    ...
    activeSectionDOMElement.parentElement.scrollIntoView({
      behavior: 'instant',
      inline: 'center',
      block: 'nearest',
    });
  ...
  return (
    <>
      <article
        className="cert-article plan-4pp"
        id={`${articleKey}`}
        aria-labelledby={`${articleKey}-article`}
      >
        <LevelTwoHeading />
        <nav
          className={`${props.pagination} ${activeCertViewer ? 'active-cert-viewer' : ''}`}
          ref={navRef}
        >
          <ol>{props.sectionArranger()}</ol>
          <ul>{props.sectionController()}</ul>
        </nav>
      </article>
    </>
  );
