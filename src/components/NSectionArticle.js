...
function NSectionArticle() {
  ...
  function handleNavButtonVisibility(buttonType) {
    if (
      (buttonType === 'first' && sectionGroupIndex < 2) ||
  ...
  function sliceIntoSectionGroup() {
    const sectionGroup = activeArticle.sections.slice(
      sectionGroupIndex * sectionsPerGroup,
      (sectionGroupIndex + 1) * sectionsPerGroup,
    );
  ...
  function sectionNavControls() {
    return (
      <>
        <li>
          <button
            aria-label="Navigate to first section group"
            className={'nav-seq-button to-the-first-nav-button'}
            disabled={handleNavButtonVisibility('first')}
            onClick={() => navigateToFirstSectionGroup(newSectionGroupIndex)}
          ></button>
        </li>
        <li>
          <button
            aria-label="Navigate to previous section group"
            className={'nav-seq-button to-the-previous-nav-button'}
            disabled={handleNavButtonVisibility('previous')}
            onClick={() => navigateToPreviousSectionGroup(newSectionGroupIndex, sectionGroupIndex)}
          ></button>
        </li>
        <li>
          <button
            aria-label="Navigate to next section group"
            className={'nav-seq-button to-the-next-nav-button'}
            disabled={handleNavButtonVisibility('next')}
            onClick={() => navigateToNextSectionGroup(newSectionGroupIndex, sectionGroupIndex)}
          ></button>
        </li>
        <li>
          <button
            aria-label="Navigate to last section group"
            className={'nav-seq-button to-the-last-nav-button'}
            disabled={handleNavButtonVisibility('last')}
            onClick={() => navigateToLastSectionGroup(newSectionGroupIndex)}
          ></button>
        </li>
      </>
    );
