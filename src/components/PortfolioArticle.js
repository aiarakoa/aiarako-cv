...
function PortfolioArticle() {
  ...
  const { activeArticle, sectionsCount, carouseledSectionKeys, setCarouseledSectionKeys } =
    useArticle();
  const [activeSectionKey, setActiveSectionKey] = useState(activeArticle.sections[0].key);
  const [previouslyActiveSectionKey, setPreviouslyActiveSectionKey] = useState(
    activeArticle.sections[0].key,
  );
  ...
  function rotateCarousel(direction) {
    if (
      sectionsCount < 2 ||
      (activeSectionKey === carouseledSectionKeys[0] && direction === 'next') ||
      (activeSectionKey === carouseledSectionKeys[sectionsCount - 1] && direction === 'previous')
    ) {
      return;
    }
    if (direction === 'previous') {
      setCarouseledSectionKeys((carouSections) => [
        carouSections[sectionsCount - 1],
        ...carouSections.slice(0, sectionsCount - 1),
      ]);
    } else {
      setCarouseledSectionKeys((carouSections) => [
        ...carouSections.slice(1, sectionsCount),
        carouSections[0],
      ]);
    }
      ...
      <>
        <li>
          <button
            ...
            disabled={handleNavButtonVisibility()}
            onClick={navigateToPreviousSectionGroup}
          ></button>
        </li>
        <li>
          <button
            ...
            disabled={handleNavButtonVisibility()}
            onClick={navigateToNextSectionGroup}
          ></button>
        </li>
      </>
    );
