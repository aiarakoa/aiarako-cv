...
function Section(props) {
    ...
    return (
      <>
        <time dateTime={thisSection.timeFrameStart}>
          {formatYearMonth(
            thisSection.timeFrameStart,
            '-',
            thisSection.timeFrameTextualMonth,
            selectedLanguage,
          )}
        </time>
  ...
  function mapParagraphs() {
    return thisSection.paragraphs.map((paragraph) => (
      <p
        id={`${props.sectionKey}-p${paragraphCount++}`}
        key={`${props.sectionKey}-p${paragraphCount++}`}
      >
        {paragraph.content}
      </p>
    ));
  ...
  function mapDescriptionDetails(descriptionDetails) {
    return descriptionDetails.map((detail, detailIndex) => (
      <dd key={`${props.sectionKey}-dd-${detailIndex}`}>{processDetailContent(detail)}</dd>
    ));
  }
        ...
        <details open={presentationMode === 'laptop'}>
          <summary>
            <span
              role="heading"
              aria-level="3"
              id={`${props.sectionKey}-section-heading`}
              className="details-summary"
            >
              <span className="laptop-span">{thisSection.title}</span>
              <span className="mobile-span">{thisSection.mobileTitle}</span>
            </span>
            <p className="organisation-subtitle">{thisSection.organisation}</p>
            <p className="time-frame-subtitle">{thisSection.timeFrameExists ? mapDate() : null}</p>
          </summary>
          {mapParagraphs()}
          {mapDescriptionList()}
